 ...
 */
  ...
  * -------------------------------------------------------------
  */
mamba_void bErrorPrint(mamba_char* fmt, ...);
mamba_void displayBuffer(BufferReader* ptr_Buffer);
mamba_long getFileSize(mamba_char* fname);
mamba_intg isNumber(const mamba_char* ns);
mamba_void startReader(mamba_char*, mamba_char*, mamba_char, mamba_intg, mamba_intg);
...
mamba_intg mainReader(mamba_intg argc, mamba_char** argv) {
	...
	mamba_char* program = argv[0];
	mamba_char* input = argv[2];
	mamba_char mode = MODE_FIXED;
	mamba_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
mamba_void startReader(mamba_char* program, mamba_char* input, mamba_char mode, mamba_intg size, mamba_intg increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	mamba_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	mamba_char symbol;			/* symbol read from input file */
