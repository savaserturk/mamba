...
mamba_intg errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern mamba_intg syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern mamba_intg line; /* source code line number - defined in scanner.c */
...
extern mamba_void startParser(mamba_void);
extern mamba_intg startScanner(ReaderPointer sc_buf);

static mamba_void printParserError(mamba_char* fmt, ...);
static mamba_void displayParser(ReaderPointer ptrBuffer);
static mamba_long getParserFileSize(mamba_char* fname);
static mamba_void callGarbageCollector(mamba_void);
...
mamba_intg mainParser(mamba_intg argc, mamba_char** argv) {
	...
	mamba_intg loadsize = 0; /*the size of the file loaded in the buffer */
...
mamba_void printParserError(mamba_char* fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(mamba_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
mamba_long getParserFileSize(mamba_char* fname) {
	FILE* input;
	mamba_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
mamba_void displayParser(ReaderPointer ptrBuffer) {
...
mamba_void callGarbageCollector(mamba_void) {
